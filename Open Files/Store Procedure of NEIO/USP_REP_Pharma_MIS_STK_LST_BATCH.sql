IF EXISTS(SELECT [NAME] FROM SYSOBJECTS WHERE XTYPE='P' AND [NAME]='USP_REP_Pharma_MIS_STK_LST_BATCH')
BEGIN
	DROP PROCEDURE [USP_REP_Pharma_MIS_STK_LST_BATCH]
END
GO

CREATE PROCEDURE [USP_REP_Pharma_MIS_STK_LST_BATCH]
...
,IQTY_ST =SUM(CASE WHEN A.BEH in ('ST','DC') AND A.PMKEY='-' AND A.DATE>=@SDATE THEN A.QTY ELSE 0 END)			--Added by Shrikant S. on 28/11/2018 for Installer 2.1.0
--,IQTY_ST =SUM(CASE WHEN A.BEH in ='ST'  AND A.PMKEY='-' AND A.DATE>=@SDATE THEN A.QTY ELSE 0 END)		--Commented by Shrikant S. on 28/11/2018 for Installer 2.1.0
...
,IQTY_OT=SUM(CASE WHEN A.BEH NOT IN ('ST','IP','II','PR','DC') AND A.PMKEY='-' AND A.DATE>=@SDATE THEN A.QTY ELSE 0 END)		--Added by Shrikant S. on 28/11/2018 for Installer 2.1.0
--,IQTY_OT=SUM(CASE WHEN A.BEH NOT IN ('ST','IP','II','PR') AND A.PMKEY='-' AND A.DATE>=@SDATE THEN A.QTY ELSE 0 END)		--Commented by Shrikant S. on 28/11/2018 for Installer 2.1.0
...
DROP TABLE #TITEM
--DROP TABLE #TITEM1			--Commented by Shrikant S. on 28/11/2018 for Installer 2.1.0
