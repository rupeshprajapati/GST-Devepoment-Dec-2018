...
namespace WindowsFormsApplication1
{
    public partial class frmImpMain : uBaseForm.FrmBaseForm
    {
        ...
        private void btnImportFrom_Click(object sender, EventArgs e)
        {
            ...
            oSelectPop.Icon = this.pFrmIcon;  //Added by Priyanka B on 11012018 for Bug-30849 & Bug-31076
                ...
                while (vTables.IndexOf("<<") > -1)
                {
                    ...
                    try//Added by Archana K on 07/03/14 for Bug-20512 
                    {
                        ...
                        if (File.Exists(fName))//Changed by Archana K. on 03/06/13 for Bug-5837
                        {
                            ...
                            string SqlTblStruct = ""; //"Select * from " + vTable + " where 1=2";
                            SqlTblStruct = "SELECT o.name,B.name 'Column Name',C.Name 'Data type',B.max_length 'Max Length',B.precision,B.scale,B.is_nullable";
                            SqlTblStruct = SqlTblStruct + " ,ISNULL(i.is_primary_key, 0) 'Primary Key'";
                            SqlTblStruct = SqlTblStruct + " ,FLDSTRU='['+B.[name]+'] '+ CASE";
                            SqlTblStruct = SqlTblStruct + " WHEN C.[NAME]='TEXT' THEN C.[NAME]";
                            SqlTblStruct = SqlTblStruct + " WHEN C.[NAME]='INT' THEN C.[NAME]";
                            SqlTblStruct = SqlTblStruct + " WHEN C.[NAME]='TINYINT' THEN C.[NAME]";
                            SqlTblStruct = SqlTblStruct + " WHEN C.[NAME]='SMALLDATETIME' THEN C.[NAME]";
                            SqlTblStruct = SqlTblStruct + " WHEN C.[NAME]='DATETIME' THEN C.[NAME]";
                            SqlTblStruct = SqlTblStruct + " WHEN C.[NAME]='BIT' THEN C.[NAME]";
                            SqlTblStruct = SqlTblStruct + " WHEN C.[NAME]='DECIMAL' THEN C.[NAME]+'('+CAST(B.[precision] AS VARCHAR)+','+CAST(B.[SCALE] AS VARCHAR)+')'";
                            SqlTblStruct = SqlTblStruct + " WHEN C.[NAME]='NUMERIC' THEN C.[NAME]+'('+CAST(B.[precision] AS VARCHAR)+','+CAST(B.[SCALE] AS VARCHAR)+')'";
                            SqlTblStruct = SqlTblStruct + " WHEN C.[NAME]='VARCHAR' THEN C.[NAME]+'('+CAST(B.[MAX_LENGTH] AS VARCHAR)+')'";
                            SqlTblStruct = SqlTblStruct + " WHEN C.[NAME]='CHAR' THEN C.[NAME]+'('+CAST(B.[MAX_LENGTH] AS VARCHAR)+')' END";
                            SqlTblStruct = SqlTblStruct + " FROM SYS.COLUMNS B";
                            SqlTblStruct = SqlTblStruct + " INNER JOIN SYS.TYPES C ON (C.USER_TYPE_ID = B.USER_TYPE_ID) ";
                            SqlTblStruct = SqlTblStruct + " INNER JOIN SYSOBJECTS O ON (O.ID = B.OBJECT_ID) ";
                            SqlTblStruct = SqlTblStruct + " LEFT OUTER JOIN sys.index_columns ic ON (ic.object_id = B.object_id AND ic.column_id = B.column_id) ";
                            SqlTblStruct = SqlTblStruct + " LEFT OUTER JOIN sys.indexes i ON (ic.object_id = i.object_id AND ic.index_id = i.index_id) ";
                            SqlTblStruct = SqlTblStruct + " WHERE O.NAME = '" + vTable + "'";
                            DataSet dsTblStruct = new DataSet();
                            dsTblStruct = oDataAccess.GetDataSet(SqlTblStruct, null, 20);
                            //oDataAccess.ExecuteSQLStatement(SqlTblStruct, null, Timeout, false);
                            ...
                            //string DDL = SqlTableCreator.GetCreateFromDataTableSQL(TargetTableName, dataSet.Tables[0]);  //Commented by Priyanka B on 05012018 for Bug-31076
                            string DDL = SqlTableCreator.GetCreateFromDataTableSQL(TargetTableName, dsTblStruct.Tables[0], dataSet.Tables[0]);  //Modified by Priyanka B on 05012018 for Bug-31076
                            ...
                            //SqlStr = " execute " + dr["sp_name"].ToString().Trim() + " '" + dr["code"].ToString().Trim() + "','";  //Commented by Priyanka B on 06012018 for Bug-31076
                            SqlStr = "Set DateFormat DMY execute " + dr["sp_name"].ToString().Trim() + " '" + dr["code"].ToString().Trim() + "','";  //Modified by Priyanka B on 06012018 for Bug-31076
