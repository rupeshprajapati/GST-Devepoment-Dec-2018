...
namespace udMarketingCompanyMast
{
    public partial class Form_upload : uBaseForm.FrmBaseForm
    {
        ...
        private void setRemark()
        {
            foreach (DataRow row1 in dtExcel.Rows)
            {
                ...
                if (row1["Status"].ToString() != "Deleted" && row1["Status"].ToString() != "Inserted" && row1["Status"].ToString() != "Not Inserted")
                {
                    ...
                    SqlStr = "select * from pRetDrugMarketingMast where Name='" + Name + "' and Name!=''";
        ...
        private void bindGrid(DataTable dtExcelTemp)
        {
            ...
            dataGridView1.Columns[24].DataPropertyName = "Status";
            dataGridView1.Columns[25].DataPropertyName = "Error";
            dataGridView1.AutoGenerateColumns = false;
        ...
        private void btnDownload_Click(object sender, EventArgs e)
        {
            ...
            if (dtRecord != null)
            {
                string file_path = getFileName("\\Marketing_Company_Format_");
                if (file_path != "")
                {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (dtExcel.Rows.Count > 0)
            {
                string file_path = getFileName("\\Marketing_Company_Log_");
                if (file_path != "")
                {
                    progressBar1.Value = 20;
                    DataView view = new DataView(dtExcel);
                    DataTable dtLogRecord = view.ToTable(false, "Name", "Code", "ContactPerson", "Designation", "Address1", "Address2", "Address3", "Area", "Zone", "CITY", "District", "State", "Zip", "Country", "OfficeTel", "Fax", "ResiTel", "CellNo", "EmailId", "DrugLicNo", "DrugLicExpDate", "FSSAINo", "FSSAIExpDate", "Status", "Error");//Add RRG
        ...
        private void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataRow dr in dtUpData.Rows)
            {
                ...
                if (dr["Status"].ToString() == "New Record" && dr["select"].ToString() == "True")
                {
                    ...
                    error_message = validate(mName, stateCode, Country_id, zip, office, phoner, cellno, email, mcode, prDrugExpDate, prFoodExpDate);
                    if (stateCode == "0")
                    {
                        stateCode = "";
                    }
        ...
        public string validate(string name, string stateCode, int countryid, string zip, string office, string phoner, string cellno, string email, string mcode, DateTime prDrugExpDate, DateTime prFoodExpDate)
        {
            error_message = "";
            int n;
            ...
            {
                error_message = "Marketing Company Name cannot be empty.";

            }
            else if (mcode.Length > 10)
            {
                error_message = "Code sholud be less than or equal 10 Character.";
            }
            else if ((dtManufacturer.Rows.Count > 0) && this.pAddMode == true)
            {
                error_message = "Marketing Company Name already exist.";
            }
            else if (stateCode == "")
            {
