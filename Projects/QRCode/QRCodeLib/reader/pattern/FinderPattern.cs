...
using InvalidVersionInfoException = Udyog.QRCode.ExceptionHandler.InvalidVersionInfoException;
...
namespace Udyog.QRCode.Codec.Reader.Pattern
{
	...
	public class FinderPattern
	{
		...
		internal static Line[] findLineAcross(bool[][] image)
		{
			...
			canvas.drawLines(foundLines, Udyog.QRCode.Codec.Util.Color_Fields.LIGHTGREEN);
			return foundLines;
		}
		...
		internal static Point[] getCenter(Line[] crossLines)
		{
			...
			{
				canvas.drawPolygon(foundPoints, Udyog.QRCode.Codec.Util.Color_Fields.RED);
				return foundPoints;
			}
			else
		...
		internal static int calcExactVersion(Point[] centers, int[] angle, int[] moduleSize, bool[][] image)
		{
			...
			canvas.drawPoints(points, Udyog.QRCode.Codec.Util.Color_Fields.RED);
