using System;
using QRCodeDecoder = Udyog.QRCode.Codec.QRCodeDecoder;
using Udyog.QRCode.Codec.Reader;
using AlignmentPatternNotFoundException = Udyog.QRCode.ExceptionHandler.AlignmentPatternNotFoundException;
using InvalidVersionException = Udyog.QRCode.ExceptionHandler.InvalidVersionException;
using Udyog.QRCode.Geom;
using Udyog.QRCode.Codec.Util;

namespace Udyog.QRCode.Codec.Reader.Pattern
{
	...
	public class AlignmentPattern
	{
		...
		internal static Point[][] getCenter(bool[][] image, FinderPattern finderPattern, Point[][] logicalCenters)
		{
			...
			canvas.drawCross(centers[0][0], Udyog.QRCode.Codec.Util.Color_Fields.BLUE);
			
			axis.Origin = finderPattern.getCenter(FinderPattern.UR);
			centers[sqrtCenters - 1][0] = axis.translate(- 3, 3);
			canvas.drawCross(centers[sqrtCenters - 1][0], Udyog.QRCode.Codec.Util.Color_Fields.BLUE);
			
			axis.Origin = finderPattern.getCenter(FinderPattern.DL);
			centers[0][sqrtCenters - 1] = axis.translate(3, - 3);
			canvas.drawCross(centers[0][sqrtCenters - 1], Udyog.QRCode.Codec.Util.Color_Fields.BLUE);
			...
			for (int y = 0; y < sqrtCenters; y++)
			{
				for (int x = 0; x < sqrtCenters; x++)
				{
					...
					if (y == 0)
					{
						...
						canvas.drawCross(centers[x][y], Udyog.QRCode.Codec.Util.Color_Fields.RED);
					}
					else if (x == 0)
					{
					...
					canvas.drawCross(centers[x][y], Udyog.QRCode.Codec.Util.Color_Fields.BLUE);
					canvas.drawLine(new Line(tmpPoint, centers[x][y]), Udyog.QRCode.Codec.Util.Color_Fields.LIGHTBLUE);
