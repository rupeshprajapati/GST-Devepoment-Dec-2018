...
namespace Udyog.QRCode.Codec
{
    public class QRCodeEncoder
    {
        ...
        public virtual string UdyogEncodeBarcode(string content)
        {
            Bitmap bmp;
            if (QRCodeUtility.IsUniCode(content))
            {
                bmp= Encode(content, Encoding.Unicode);
            }
            else
            {
                bmp= Encode(content, Encoding.ASCII);
            }
            string filename = Path.Combine(System.IO.Path.GetTempPath(), System.IO.Path.GetTempFileName());
            filename = filename.Replace(".tmp", ".bmp");
            bmp.Save(filename);
