...
*!*	_custdatasessionid = _Screen.ActiveForm.DataSessionId		&& Commented by Shrikant S. on 16/12/2017 for auto updater 13.0.5
_custdatasessionid = _custvouobj.DataSessionId					&& Added by Shrikant S. on 16/12/2017 for auto updater 13.0.5
...
&& Added by Shrikant S. on 21/12/2016 for GST		&& Start
			If Type('_custvouobj.mainalias') = 'C'
				If Upper(_custvouobj.mainalias) <> 'MAIN_VW'
					Return
				Endif
			ENDIF

&& Commented by Shrikant S. on 15/11/2017 for GST 	&& Start Bug-30825		
...
*!*				
&& Commented by Shrikant S. on 15/11/2017 for GST 	&& End Bug-30825		
...
&& Added by Shrikant S. on 14/11/2017 for GST Bug-30825		&& Start
			If Inlist(main_vw.Entry_ty,"BP","CP")
				custsql_str = "Select Isservice From It_mast Where It_code= ?Item_vw.It_code"
				custsql_con = _custsqlconobj1.dataconn([EXE],company.dbname,custsql_str,"_tmpdata3","custnhandle",_custdatasessionid)
				If custsql_con > 0 And Used('_tmpdata3')
					IF !_tmpdata3.Isservice
						RETURN 
					endif 
				Endif
			Endif
&& Added by Shrikant S. on 14/11/2017 for GST Bug-30825		&& End			
			_custExemptParty = .F.
...
*!*				custsql_str = custsql_str + " Where Name = ?Acdetalloc_vw.Serty and (?Main_vw.Date Between sdate and edate)"		&& Commented by Shrikant S. on 06/10/2016 for GST
			...
			custsql_con = _custsqlconobj1.dataconn([EXE],company.dbname,custsql_str,"_tmpdata2","custnhandle",_custdatasessionid)		&& Changed the name for the cursor to 2 for Bug-30825
			If custsql_con > 0 And Used('_tmpdata2')									&& Changed the name for the cursor to 2 for Bug-30825
				_PerPayReceiver = 0
				If !Empty(_tmpdata2.PerPayReceiver)
					_PerPayReceiver = Evaluate(_tmpdata2.PerPayReceiver)				&& Changed the name for the cursor to 2 for Bug-30825
				Endif
...
*!*					IF (INLIST(LOWER(_custvouobj.accregistatus),"unregistered","compounding","sez","eou")  OR INLIST(LOWER(_custvouobj.taxapplarea),"out of country")) AND INLIST(main_vw.Entry_ty,"E1","C6","D6","BP","CP")	&& Commented by Shrikant S. on 12/10/2017 for GST	
*!*					IF (INLIST(LOWER(_custvouobj.accregistatus),"unregistered","compounding","sez","eou")  OR INLIST(LOWER(_custvouobj.taxapplarea),"out of country")) AND (INLIST(main_vw.Entry_ty,"E1","C6","D6","BP","CP") OR (Lcode_vw.Isservitem=.t. AND Lcode_vw.Iotrans=1))		&& Added by Shrikant S. on 12/10/2017 for GST		&& Commented by Shrikant S. on 28/10/2017 for GST
*!*					IF (INLIST(LOWER(_custvouobj.accregistatus),"unregistered","compounding","sez")  OR INLIST(LOWER(_custvouobj.taxapplarea),"out of country")) AND (INLIST(main_vw.Entry_ty,"E1","C6","D6","BP","CP") OR (Lcode_vw.Isservitem=.t. AND Lcode_vw.Iotrans=1))		&& Added by Shrikant S. on 28/10/2017 for GST	&& Commented by Shrikant S. on 02/11/2017 for GST Bug-30830
				IF (INLIST(LOWER(_custvouobj.accregistatus),"unregistered","compounding","sez","eou")  OR INLIST(LOWER(_custvouobj.taxapplarea),"out of country")) AND (INLIST(main_vw.Entry_ty,"E1","C6","D6","BP","CP") OR (Lcode_vw.Isservitem=.t. AND Lcode_vw.Iotrans=1))		&& Shrikant S. on 02/11/2017 for GST Bug-30830
